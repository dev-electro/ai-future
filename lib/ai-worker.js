...
// Configure environment for purely remote fetching
env.allowLocalModels = false;
env.allowRemoteModels = true;
env.backends.onnx.wasm.numThreads = 1;
// Route around jsdelivr adblocker bans by using unpkg explicitly
env.backends.onnx.wasm.wasmPaths = 'https://unpkg.com/onnxruntime-web@1.26.0-dev.20260416-b7804b056c/dist/';
// Disable browser cache because the user's cache might be corrupted
env.useBrowserCache = false;

// Intercept fetch to debug exactly which URL is failing
const originalFetch = env.fetch;
env.fetch = async (url, init) => {
  try {
    const response = await originalFetch(url, init);
    if (!response.ok) {
      console.error(`[Worker] HTTP ${response.status} fetching: ${url}`);
    }
    return response;
  } catch (err) {
    console.error(`[Worker] Native fetch failed for URL: ${url}`, err);
    throw new Error(`Failed to fetch ${url}. Reason: ${err.message}`);
  }
};
...
async function load(modelId, onProgress) {
  ...
  try {
    ...
    console.error("[Worker] from_pretrained failed:", err);
    throw new Error(`Model download failed. If you have an adblocker (like uBlock) or Brave Shields enabled, please disable them for this site so the model weights can be downloaded from HuggingFace. Original error: ${err.message}`);
  }
}
...
self.addEventListener('message', async (event) => {
  ...
  // Keep track of download progress per file
  const downloadState = new Map();

  const onProgress = (info) => {
    // We only care about file downloads for the progress bar
    if (info.status === 'initiate' || info.status === 'download' || info.status === 'progress' || info.status === 'done') {
      const fileId = info.name || info.file || 'unknown';
      
      if (!downloadState.has(fileId)) {
        downloadState.set(fileId, { loaded: 0, total: 0 });
      }
      
      const fileState = downloadState.get(fileId);
      if (info.total) fileState.total = info.total;
      if (info.loaded) fileState.loaded = info.loaded;
      if (info.status === 'done') fileState.loaded = fileState.total; // ensure 100% on done

      // Calculate aggregate totals
      let totalLoaded = 0;
      let totalExpected = 0;
      for (const state of downloadState.values()) {
        totalLoaded += state.loaded;
        // If a file doesn't have a total yet, estimate it (e.g. config files are small, weights are large)
        totalExpected += state.total || (state.loaded + 1048576); 
      }
      ...
      emit('progress', {
        data: {
          ...
          progress: totalExpected > 0 ? Math.round((totalLoaded / totalExpected) * 100) : 0,
          loaded: totalLoaded,
          total: totalExpected,
        },
      });
