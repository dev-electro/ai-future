...
const PROVIDER_TIMEOUT_MS = 20_000; // 20s per provider
...
// IMPORTANT: Gemini API MUST be first — it is the primary provider for the challenge.
const PROVIDERS = [
    {
    ...
    {
        id: "Nebius:Gemma-4-31B",
        name: "Nebius",
        url: "https://api.studio.nebius.ai/v1/chat/completions",
        model: "google/gemma-4-31b-it",
        getKey: () => process.env.NEBIUS_API_KEY,
    },
    {
        id: "Nebius:Gemma-4-26B",
        name: "Nebius",
        url: "https://api.studio.nebius.ai/v1/chat/completions",
        model: "google/gemma-4-26b-a4b-it",
        getKey: () => process.env.NEBIUS_API_KEY,
...
export async function fetchWithFallback(systemPrompt, userMessage, clientSignal) {
    ...
    return {
        data: null,
        provider: null,
        error: `All analysis providers failed or timed out. Last error: ${lastError?.message || "Unknown"}`,
    };
}
