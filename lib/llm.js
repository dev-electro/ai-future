...
const TIMEOUT_FLASH_MS   = 12_000;  // gemini-2.0-flash: typically 1-3s
const TIMEOUT_PRIMARY_MS = 15_000;  // Gemma MoE: slower due to thinking tokens
const TIMEOUT_FALLBACK_MS = 8_000;  // Last-resort fallbacks
const MAX_TOKENS  = 2000;
const TEMPERATURE = 0.1;

/**
 * Provider chain — ordered by speed + reliability:
 * 1. gemini-2.0-flash  — sub-3s, same key, rock solid
 * 2. gemma-4-26b-a4b   — Gemma MoE (slower due to thinking tokens, ~8-15s)
 * 3. gemma-4-31b-it    — Gemma dense (slower still)
 * 4. OR:gemma-4-31b    — OpenRouter, separate daily quota
 * 5. OR:gemma-4-26b    — OpenRouter last resort
 * Total hard budget: 12+15+8+8+8 = 51s, but Vercel kills at 30s.
 * In practice flash wins in <3s on >99% of requests.
 */
const PROVIDERS = [
    {
        id:        "Gemini:gemini-2.0-flash",
        url:       "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions",
        model:     "gemini-2.0-flash",    // Primary: fast, free, no thinking tokens
        getKey:    () => process.env.GEMINI_API_KEY,
        timeoutMs: TIMEOUT_FLASH_MS,
    },
    {
        id:        "Gemini:gemma-4-26b-a4b-it",
        url:       "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions",
        model:     "gemma-4-26b-a4b-it",  // Gemma MoE fallback
        getKey:    () => process.env.GEMINI_API_KEY,
    ...
    },
    {
        ...
        url:       "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions",
        model:     "gemma-4-31b-it",      // Gemma dense fallback
        getKey:    () => process.env.GEMINI_API_KEY,
