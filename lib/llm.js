...
const TIMEOUT_PRIMARY_MS  = 22_000;  // Gemini 31B — generous window
const TIMEOUT_FALLBACK_MS =  5_000;  // All other providers — fast or skip
const MAX_TOKENS = 1200;             // Compact output — fast to generate, still schema-complete
...
const PROVIDERS = [
    {
        ...
        model: "gemma-4-31b-it",           // ✓ from ai.google.dev/gemma/docs/core/gemma_on_gemini_api
    ...
    },
    {
        ...
        url:   "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions",
        model: "gemma-4-26b-a4b-it",       // ✓ from ai.google.dev/gemma/docs/core/gemma_on_gemini_api
        getKey: () => process.env.GEMINI_API_KEY,
    ...
    },
    {
        ...
        model: "google/gemma-4-31b-it:free",      // ✓ $0 confirmed via /api/v1/models
    ...
    },
    {
        ...
        url:   "https://openrouter.ai/api/v1/chat/completions",
        model: "google/gemma-4-26b-a4b-it:free",  // ✓ $0 confirmed via /api/v1/models
        getKey: () => process.env.OPENROUTER_API_KEY,
        timeoutMs: TIMEOUT_FALLBACK_MS,
    },
];
        ...
        try {
            ...
            const res = await fetch(provider.url, {
                ...
                body: JSON.stringify({
                    ...
                    // response_format supported by both Gemini OpenAI compat and OpenRouter
            ...
            try {
                ...
                console.error(`[LLM] ✗ ${provider.id} invalid JSON: ${rawText.slice(0, 150)}`);
